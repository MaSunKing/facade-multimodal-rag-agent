...
发布目录实际完成75题离线链路复跑，全部Prompt预算通过、运行错误0，完整支持Recall@5为38/45、MRR为0.5733，与公开结果一致。14份官方原件SHA校验及原生金标来源绑定通过；原件不随仓库再分发。固定题目与文件字节不变，重建Canonical差异独立记录，不改历史冻结Manifest。

`verify_unified_publication.py`核算已发布结果、固定题目、源码指纹和扫描页fixture；`verify_unified_public_eval.py`检查重新运行的输出，两者职责不同。公开源码指纹统一CRLF→LF，冻结题目/自建来源按原始字节保存，便于跨平台核验。

本轮源码回归325项：271通过、54依赖明确跳过、0失败/错误。没有加载8B，没有重新构建前端，没有把企业混合探针或一页OCR并入75题成绩。主集合仍属已知开发回归，详见[统一协议](UNIFIED_EVALUATION.md)。

CI加入统一结果与修复结果的指纹校验；远端执行结果以Actions为准。CI的CPU接口作业显式关闭GPU混合检索/OCR，避免下载模型，不改变用户运行配置的默认开启状态，也不能代替本机的独立混合检索/OCR验收。

## 原生关系修复后补测（2026-09-17）

325项CPU回归：271通过、54显式跳过、0失败/错误；FastAPI进程内HTTP Smoke 20/20通过。新增单位跨块/跨列/章节切换、未知前缀拒绝、数据行不升格表头及查询排序回归。原题45道及1800预算压力对照见[NATIVE_RELATION_REPAIR.md](NATIVE_RELATION_REPAIR.md)。本次不加载8B生成模型，不代表生产完整回答质量；下面发布验证和75题为保留的历史记录。
