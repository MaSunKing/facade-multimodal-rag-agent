...
## 真实8B Demo与契约修复（2026-09-17）

重启核实后的本地服务，3个自建原生附件案例均实际进行8B生成。修复前均被`GROUNDING_FAILED`拦截；复现并修复覆盖Prompt与严格字段集合冲突后，相同问题及相同原件SHA均返回回答和引用。通用修复只允许受限`answer_aspect_coverage`扩展，未知字段/无效引用仍拒绝；实际图片顺序直接加入审计，不从UI结果重建。

6项新增契约测试通过。源码回归331项：277通过、54明确跳过、0失败/错误。修复后75题完整复跑，预算75/75、错误0，所有原指标不变；原始执行指纹保留，当前发布指纹单独更新并注明范围。历史paired基线未冒充本次重跑。

真实Demo耗时54.223/28.731/24.646秒，未联网或调用Judge，走附件快捷计划。人工内容检查发现联合产品介绍不足、版本差异概括不准、视觉引用/来源标签有缺口，见[实际记录](../examples/real_agent_demo/README.md)。不声称3题全对或复杂Planner已经验收。此轮未重新构建前端。
